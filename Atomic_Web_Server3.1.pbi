DeclareModule Atomic
	...
	Prototype Atomic_Server_pURIHandler(*request.Request)
	Prototype Atomic_Server_pPostGet(*request.Request) 
	Prototype Atomic_server_pPreprocess(*request.Request) 
	
	Structure Handlers 
		*pt.Atomic_Server_pURIHandler 
	EndStructure  
	
	Structure Log_window 
		Window.i
		Gadget.i
	EndStructure  
	...
	Structure Server
		...
		Map URIHandlers.Handlers(128) 
		pCBPost.Atomic_Server_pPostGet    
		pCBGet.Atomic_Server_pPostGet 
		Quit.i
	EndStructure
	
	CompilerIf Defined(EzPack, #PB_Module)
		Structure Client 
			ID.i 
			ServerId.i
			tid.i
...
Module Atomic
	...
	CompilerIf Not #PB_Compiler_Thread 
		CompilerWarning "Compile with thread safe!"
	CompilerEndIf  
	...
	Procedure Atomic_Server_Init_MimeTypess(*Atomic_server.Server) 
	...
	;-Declares 
	Declare Atomic_Server_Init(Title.s,WWWDirectory.s,Ip.s="127.0.0.1",domain.s="",Port=80,tls=0,IpVer=#PB_Network_IPv4,maxClients=1000,*pCBPost=0,*pCBGet=0,CacheAge=0) 
	Declare Atomic_Server_Start(Server,window=-1,Blog=0)                                                 
	Declare Atomic_Server_Thread(*Atomic_server.Server) 
	Declare Atomic_Server_ProcessRequest(*Atomic_Client.Client)                                    
	Declare Atomic_Server_BuildRequestHeader(*request.Request,*FileBuffer, FileLength, ContentType.s,Status.i=200,gzip=0,bCache=1)
	Declare Atomic_Server_Exit(Server)                                                  
	Declare Atomic_Server_Index(*request.Request) 
	Declare Atomic_Server_Error(*request.Request) 
	Declare Atomic_Server_Favicon(*request.Request)
	...
	Procedure Atomic_Server_FreeClient(*client.Client,bclose=1) 
		Protected *Atomic_Server.Server = *client\ServerId  
	...
	Procedure Atomic_Server_Thread(*Atomic_server.Server) 
		
		Protected  ServerEvent, Result, ClientID, MaxRequest,pos,epos,*msg
		Protected  *buffer 
		Protected  NewMap clients.Client(*Atomic_server\maxclients)
		Protected  atomicserver, key.s, request.s,req.s,*err,timeout  
		Protected  head.s, ContentLen,len
		
		If *Atomic_server\tls = 0 ;Port <> 443 
			atomicserver = CreateNetworkServer(#PB_Any,*Atomic_Server\Port,#PB_Network_TCP | *Atomic_server\IpVer,*Atomic_server\IP)  
		Else   
			atomicserver = CreateNetworkServer(#PB_Any,*Atomic_Server\Port,#PB_Network_TCP | *Atomic_server\IpVer | #PB_Network_TLSv1_3,*Atomic_server\IP)
			;atomicserver = CreateNetworkServer(#PB_Any,*Atomic_Server\Port,#PB_Network_TCP | *Atomic_server\IpVer ,*Atomic_server\IP)  
		EndIf 
	...
	Procedure Atomic_Server_deflate(*request.Request,*input,len) 
		
		Protected  *atomic_client.Client = *request\clientID 
	...
	Procedure Atomic_Server_Preprocess(*input,len,*request.Request) 
	...
	Procedure Atomic_Server_Send(*request.Request,*buffer,len,lock=1) 
		
		Protected  *atomic_server.Server = *request\Serverid  
		Protected  *atomic_client.Client = *request\clientID 
	...
	Procedure Atomic_Server_ProcessURIRequest(server,*request.Request,Requestfile.s) 
		
		Protected *data,FileLength,*FileBuffer,*BufferOffset,ContentType.s=""
		Protected outpos,fulllen,trylen,sendlen
		Protected  *atomic_server.Server = server 
		Protected  *atomic_client.Client = *request\clientID 
	...
	Procedure Atomic_Server_GetCookies(*request.Request) ;internal function retrives cookies 
		
		Protected *client.Client = *request\clientID   
	...
	Procedure Atomic_Server_GetRequestHeaders(*request.Request) ;internal gets request headers 
	...
	Procedure Atomic_Server_SetResponceHeader(*request.Request,key.s,value.s) ;faciltates adding custom header fields 
	...
	Procedure Atomic_Server_GetParameters(*request.Request)
	...
	Procedure Atomic_Server_Reverse_Proxy(*request.Request) 
	...
	Procedure Atomic_Server_ProcessRequest(*Atomic_Client.Client)
		
		Protected RequestedFile.s="", FileLength, MaxPosition, Position,ContentType.s=""
		Protected atomic_request.Request 
		Protected *Atomic_Server.Server = *Atomic_Client\serverid 
		Protected type.s=""
		...
		Repeat 
			...
			ClearStructure(@atomic_request,Request)
			...
			If *Atomic_Client\kill = 0
				...
				If TryLockMutex(*Atomic_Client\lock)
					If FirstElement(*Atomic_Client\Requests())
						CopyStructure(@*Atomic_Client\Requests(),@atomic_request,Request)
	...
	Procedure Atomic_Server_BuildRequestHeader(*request.Request,*FileBuffer, FileLength, ContentType.s,Status.i=200,gzip=0,bcache=1)
	...
	Procedure Atomic_Server_Index(*request.Request) 
	...
	Procedure Atomic_Server_Error(*request.Request) 
	...
	Procedure Atomic_Server_Favicon(*request.Request)
	...
	Procedure.s Atomic_Server_SetCookie(*request.Request,Cookie.s,value.s,maxage.l=0,brandom=0)  ;set a client cookie 
	...
	Procedure Atomic_Server_DeleteCookie(*request.Request,Cookie.s)
	...
	Procedure Atomic_Server_Create_Pack(server) 
		CompilerIf #USEEZPACK 
			Protected *atomic_server.Server = server  
			Protected pack.EzPack::iEzPack = EzPack::New() 
	...
	Procedure Atomic_Server_ReadFile(*Atomic_Client.Client,file.s) 
	...
	Procedure Atomic_Server_Init(title.s,wwwDirectory.s,IP.s="127.0.0.1",domain.s="",port=80,tls=0,IpVer=#PB_Network_IPv4,maxclients=1000,*pCBPost=0,*pCBGet=0,CacheAge=0) 
		
		Protected *atomic_server.Server 
		*atomic_server= AllocateStructure(Server) 
		If *atomic_server 
	...
	Procedure Atomic_Server_Init_TLS(server,path.s,domain.s,CertFile.s,KeyFile.s,CaCertFile.s)
		...
		*atomic_server\CertFile = path + domain + "\" + CertFile 
		...
		Debug *atomic_server\KeyFile
		Protected fn,*pkey,len  
		fn = OpenFile(-1,path + domain + "\" + KeyFile)  
		If fn 
			len = Lof(fn)-2
			*pkey = AllocateMemory(Len) 
			ReadData(fn,*pkey,Len) 
			*atomic_server\KeyFile = PeekS(*pkey,len,#PB_UTF8)
			FreeMemory(*pkey) 
			CloseFile(fn)
		EndIf 
		
		fn = OpenFile(-1,path + domain + "\" + certFile)  
		If fn 
			len = Lof(fn)-2
			*pkey = AllocateMemory(Len) 
			ReadData(fn,*pkey,Len) 
			*atomic_server\CertFile = PeekS(*pkey,len,#PB_UTF8)
			FreeMemory(*pkey) 
			CloseFile(fn)
		EndIf 
		
		fn = OpenFile(-1,path + domain + "\" + CaCertFile)  
		If fn 
			len = Lof(fn)-2
			*pkey = AllocateMemory(Len) 
			ReadData(fn,*pkey,Len) 
			*atomic_server\caCertFile = PeekS(*pkey,len,#PB_UTF8)
			FreeMemory(*pkey) 
			CloseFile(fn)
		EndIf 
